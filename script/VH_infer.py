...
if args.overlap:
    data_root = '/media/lh/D/VH_Data/test_Overlap/' # no overlap experiments
else:
    data_root = '/media/lh/D/VH_Data/test/'


rgb_root = data_root + 'rgb/'
# edge_root = data_root + 'label_edge/' #only need when test the edge performance
...
for root, dirs, files in os.walk(rgb_root):
    for onefile in files:
        if onefile.startswith('VH_'):
            ...
            im_rgb = caffe.io.load_image(fname)
            ...
            transformed_image = transformer.preprocess('data_rgb', im_rgb)
            ...
            im_ndsm = caffe.io.load_image(fname, color=False)
            ...
            transformed_image = transformer.preprocess('data_ndsm', im_ndsm)
            in_ = transformed_image
            # shape for input (data blob is N x C x H x W), set data
